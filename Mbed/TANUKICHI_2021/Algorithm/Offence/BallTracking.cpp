...
BallTracking::BallTracking(MachinePosition &_machinePosition, BallPosition &_ballPosition) :
    machinePosition(_machinePosition),
    ballPosition(_ballPosition),
    ballX(0.015, 0, 0.0015, 0, -1.2, 1.2) {}

Move BallTracking::frontSide(Sensors sens, int speed){
    Move move;
    ...
    if(sens.cam.bAngle == None){
        ...
        if((abs(relPosX) < 150 && relPosY < 300 && abs(sens.cam.bAngle) < 15) ||
           (abs(relPosX) < 150 && 150 < relPosY && relPosY < 300)){
            ...
            int anglePlus = (r - sens.cam.bDistance) * 0.1 + 90;
...
Move BallTracking::backSide(Sensors sens, int speed){
    Move move;
