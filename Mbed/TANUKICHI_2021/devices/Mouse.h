...
typedef struct {
    ...
    int deltaX=0, deltaY=0, angle=0, delta=0;
    void apply(long &x, long &y){
        x += deltaX;
        y += deltaY;
    }
    void print(){
        printf("Mouse:deltaX=%d,deltaY=%d,positionX=%ld,positionY=%ld\t", deltaX, deltaY, positionX, positionY);
    }
} Mouse_S;
...
private:
    ...
    void write_reg(uint8_t reg_addr, uint8_t _data);
    void upload_firmware();
    
    unsigned short firmware_length = 3070;
    unsigned char firmware_data[3070] = {
...
public:
    Mouse_S data;
    Mouse(SPI &_spi, PinName _ss);
    
    void init(void);
    void getValue(Mouse_S&, int);
    void positionReset(void);
    void positionSet(long, long);
};
